...
class MeshDrawer {
	...
	draw(trans) {
		...
		// Normal coordinates - Bu kısım önemli
		gl.bindBuffer(gl.ARRAY_BUFFER, this.normalbuffer);
		...
		const lightX = Math.cos(time);
		const lightY = Math.sin(time);
...
const meshFS = `
    ...
    void main() {
        if(showTex && enableLighting) {
            ...
            // Işık yönü hesaplama (dönüşle uyumlu)
            vec3 lightDir = normalize(vec3(lightPos.x, lightPos.y, 5.0));
            
            // View direction (kameradan objeye)
            ...
            // Ambient hesaplama
            vec3 ambient_light = ambient * texColor.rgb;
            
            // Diffuse hesaplama
            vec3 diffuse = diff * texColor.rgb;
            
            // Sonuç
            vec3 result = ambient_light + diffuse;
